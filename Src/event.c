...
// This module implements an event handler that enables the STM32 to enter STOP during periods when it is waiting
...
// active while in STOP mode.  We configure this timer with an extremely long period (2 seconds) so that we can
...
// from STOP mode and an early timeout on the delay.
